...
@section('title', 'Yeni Fiyatlandırma Koşulu - Logistics')

@section('content')
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h2 class="h3 fw-bold text-dark mb-1">Yeni Fiyatlandırma Koşulu</h2>
        <p class="text-secondary mb-0">Navlun fiyat hesaplama koşulu tanımla</p>
    </div>
...
<div class="bg-white rounded-3xl shadow-sm border p-4">
    <form action="{{ route('admin.pricing-conditions.store') }}" method="POST">
        @csrf
        <h4 class="h5 fw-bold text-dark mb-3 d-flex align-items-center gap-2">
            ...
            Fiyatlandırma Koşulu
        </h4>
        <div class="row g-4">
            <div class="col-md-6">
                <label class="form-label fw-semibold">Şirket <span class="text-danger">*</span></label>
            ...
            <div class="col-md-6">
                ...
                <select name="condition_type" class="form-select @error('condition_type') is-invalid @enderror" required>
                    <option value="">Seçin</option>
                    <option value="weight_based"   {{ old('condition_type') === 'weight_based'   ? 'selected' : '' }}>Ağırlık Bazlı</option>
                    <option value="distance_based" {{ old('condition_type') === 'distance_based' ? 'selected' : '' }}>Mesafe Bazlı</option>
                    <option value="flat"           {{ old('condition_type') === 'flat'           ? 'selected' : '' }}>Sabit Ücret</option>
                    <option value="zone_based"     {{ old('condition_type') === 'zone_based'     ? 'selected' : '' }}>Bölge Bazlı</option>
                </select>
            ...
            <div class="col-md-2">
                ...
                <select name="vehicle_type" class="form-select">
                    ...
                    <option value="van"     {{ old('vehicle_type') === 'van'     ? 'selected' : '' }}>Minivan</option>
                    <option value="trailer" {{ old('vehicle_type') === 'trailer' ? 'selected' : '' }}>TIR</option>
                </select>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="route_origin" value="{{ old('route_origin') }}" class="form-control" placeholder="Boş = Tümü">
            </div>
            <div class="col-md-4">
                <label class="form-label fw-semibold">Varış Şehri</label>
                <input type="text" name="route_destination" value="{{ old('route_destination') }}" class="form-control" placeholder="Boş = Tümü">
            </div>
            ...
            <div class="col-md-3">
                ...
                <input type="number" step="0.01" name="weight_from" value="{{ old('weight_from') }}" class="form-control">
            </div>
            <div class="col-md-3">
                <label class="form-label fw-semibold">Maks. Ağırlık (kg)</label>
                <input type="number" step="0.01" name="weight_to" value="{{ old('weight_to') }}" class="form-control">
            </div>
            <div class="col-md-3">
                <label class="form-label fw-semibold">Fiyat / kg</label>
                <input type="number" step="0.0001" name="price_per_kg" value="{{ old('price_per_kg') }}" class="form-control">
            </div>

            <div class="col-md-3">
                <label class="form-label fw-semibold">Min. Mesafe (km)</label>
                <input type="number" step="0.01" name="distance_from" value="{{ old('distance_from') }}" class="form-control">
            </div>
            <div class="col-md-3">
                <label class="form-label fw-semibold">Maks. Mesafe (km)</label>
                <input type="number" step="0.01" name="distance_to" value="{{ old('distance_to') }}" class="form-control">
            </div>
            <div class="col-md-3">
                <label class="form-label fw-semibold">Fiyat / km</label>
                <input type="number" step="0.0001" name="price_per_km" value="{{ old('price_per_km') }}" class="form-control">
            </div>

            <div class="col-md-3">
                <label class="form-label fw-semibold">Sabit Ücret</label>
                <input type="number" step="0.01" name="flat_rate" value="{{ old('flat_rate') }}" class="form-control">
            </div>
            <div class="col-md-3">
                <label class="form-label fw-semibold">Min. Ücret</label>
                <input type="number" step="0.01" name="min_charge" value="{{ old('min_charge', 0) }}" class="form-control">
            </div>
        ...
        <div class="d-flex align-items-center justify-content-end gap-3 mt-4 pt-4 border-top">
            ...
            <button type="submit" class="btn btn-primary">Koşul Oluştur</button>
        </div>
    </form>
</div>
@endsection
