...
@section('title', 'Kullanıcı Düzenle - Logistics')

@section('content')
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h2 class="h3 fw-bold text-dark mb-1">Kullanıcı Düzenle</h2>
        <p class="text-secondary mb-0">{{ $user->name }} kullanıcısını düzenleyin</p>
    </div>
...
<div class="bg-white rounded-3xl shadow-sm border p-4" style="border-color: var(--bs-primary-200);">
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6">
                <label class="form-label fw-semibold text-dark">Yeni Şifre</label>
                <input type="password" name="password" class="form-control border-primary-200 focus:border-primary focus:ring-primary @error('password') is-invalid border-danger @enderror">
                <small class="text-secondary">Boş bırakılırsa şifre değiştirilmez. En az 8 karakter olmalıdır.</small>
                @error('password')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                <label class="form-label fw-semibold text-dark">Yeni Şifre (Tekrar)</label>
                <input type="password" name="password_confirmation" class="form-control border-primary-200 focus:border-primary focus:ring-primary">
            </div>
            ...
            <div class="col-12" id="system-roles-section" style="{{ $hasCustomerRole ? 'display: none;' : '' }}">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-4">
                        <div class="form-check p-3 border rounded-3xl hover:shadow-sm transition-all" style="border-color: var(--bs-primary-200);">
                            <input class="form-check-input system-role-checkbox" type="checkbox" name="roles[]" value="{{ $role->id }}" id="role_{{ $role->id }}" {{ $user->roles->contains($role->id) ? 'checked' : '' }} {{ $hasCustomerRole ? 'disabled' : '' }}>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <div class="form-check p-3 border rounded-3xl hover:shadow-sm transition-all bg-success-50" style="border-color: var(--bs-success-200);">
                            <input class="form-check-input customer-role-checkbox" type="checkbox" name="roles[]" value="{{ $role->id }}" id="role_{{ $role->id }}" {{ $user->roles->contains($role->id) ? 'checked' : '' }} {{ !$hasCustomerRole && $user->roles->whereNotIn('name', ['customer', 'customer_user', 'customer_viewer'])->isNotEmpty() ? 'disabled' : '' }}>
                            <label class="form-check-label w-100" for="role_{{ $role->id }}">
